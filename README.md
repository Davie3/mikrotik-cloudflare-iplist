...
A few notes for security-conscious users:

- The `.rsc` lists are regenerated by a GitHub Actions workflow ([update_iplists.yml](.github/workflows/update_iplists.yml)) and every change is committed to this repo — `git log` and `git blame` are the audit trail.
- The installer scripts pass `check-certificate=yes-without-crl` to `/tool fetch` so RouterOS validates GitHub's TLS certificate. If you wrote your own download script before this was added, consider adding the same flag to defeat MitM tampering.
- If you fork this repo, keep the workflow's `permissions:` scoped, keep actions pinned by SHA, and turn on Dependabot — the workflow in this repo is the reference setup.
